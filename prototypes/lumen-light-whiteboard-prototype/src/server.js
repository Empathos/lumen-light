...
export async function startServer(options) {
  ...
  app.post("/api/preso/start", async (req, res) => {
    ...
    console.log(`[lumen-light-whiteboard-prototype] preso/start: ${keywords.length} staging keyword(s) for transcription bias`);
    transcription.setSessionContext({ keywords });
...
const DEFAULT_LOG_DIR = path.join(os.homedir(), ".config", "lumen-light-whiteboard-prototype", "logs");
const CACHE_USAGE_LOG_PATH = process.env.LUMEN_LIGHT_WHITEBOARD_CACHE_LOG ?? path.join(DEFAULT_LOG_DIR, "cache.log");
const DEBUG_LOG_PATH = process.env.LUMEN_LIGHT_WHITEBOARD_DEBUG_LOG ?? path.join(DEFAULT_LOG_DIR, "debug.log");
...
export function whiteboardSystemPrompt() {
  return `You are Lumen Light Whiteboard Prototype, a real-time visual note-taking agent.
